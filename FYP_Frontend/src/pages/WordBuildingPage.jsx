...
export default function WordBuildingPage() {
    ...
    // ── Progress state — init from localStorage, overridden after Supabase sync ──
    ...
    // ── On mount: fetch completed words from Supabase and sync progress ──
    useEffect(() => {
        if (!user) return;
        ...
        const syncFromSupabase = async () => {
            ...
            try {
                ...
                if (!data || data.length === 0) return; // No remote data — keep localStorage as-is
                ...
                // Merge: union of both sources so neither overwrites the other
                ...
                <Box sx={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, minHeight: 0 }}>
                    ...
                    <Card sx={{ background: C.card, borderRadius: "24px", border: `1px solid ${C.cardBorder}`, flex: 1, minHeight: 0, display: "flex", flexDirection: "column", transition: "all 0.3s ease" }}>
                        <CardContent sx={{ p: 2, flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                            ...
                            {/* ── Completion Actions — same style as LearningPage ── */}
                            {wordCompleted && (
                                <Box
                                    sx={{
                                        p: 2,
                                ...
                                >
                                    ...
                                    {/* Action buttons — identical pattern to LearningPage */}
                                    <Box sx={{ display: "flex", gap: 1 }}>
                                        {/* Redo Word — matches "🔄 Restart" in LearningPage */}
                                        <Button
                                            size="small"
                                            variant="outlined"
                                        ...
                                        {/* Next Word → — matches "Next Letter →" in LearningPage */}
                                        <Button
                                            size="small"
                                            variant="contained"

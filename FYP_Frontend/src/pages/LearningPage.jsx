...
export default function LearningPage() {
  ...
  // ── Stats state — starts from localStorage, then gets overwritten by Supabase ──
  const [stats, setStats] = useState(() => loadStats());
  const [statsLoading, setStatsLoading] = useState(false); // true while fetching from Supabase

  // ── On mount: fetch from Supabase and merge/overwrite localStorage ──
  useEffect(() => {
    if (!user) return;
  ...
  }, [letter]); // intentionally only [letter] — we don't want stats changes to reset UI mid-session
